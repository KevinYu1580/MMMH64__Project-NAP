...
<div class="all-container">
    ...
    <div class="booking-card-btn row">
        <div class="col add-to-cart-btn">
            <button class="napBtn_fixed_filled" name="add-to-cart" form="enroll-list" type="submit" data-sid="<?= $room['sid'] ?>" onclick="addToCart(room)">
                <span>前往結帳</span>
            </button>
        </div>
        <div class="col check-bill-btn">
            <button class="napBtn_fixed_outlined" name="add-to-cart" form="enroll-list" type="submit" data-sid="<?= $room['sid'] ?>" onclick="addToCart(room)">
                <span>加入購物車</span>
            </button>
        </div>
    </div>
...
*/
    ...
    function addToCart(room) {
        const btn = $(room.currentTarget);
        const num = room.find('.num');
        //num 數量這裡是用$('.booking-detail-content').length算人數
        const days = room.find('.days');
